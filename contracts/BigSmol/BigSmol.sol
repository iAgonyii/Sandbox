...
interface ISmolPool {
    ...
    function adjustTotalSupply(int256 totalSupplyDifference) external;
}
...
contract BigSmol is Ownable, ReentrancyGuard {
    ...
    function handleRandomRebase() public {
    ...
    function getPoolData() external view returns (
        ...
        if (poolOneTotalSupply == 0 && poolTwoTotalSupply == 0)
            return (0, 0, 0, 0, recentResults, poolOpen, rebaseReady, rebaseRequested);

        if (poolOneTotalSupply == 0)
            return (0, 0, 100, 0, recentResults, poolOpen, rebaseReady, rebaseRequested);

        if (poolTwoTotalSupply == 0)
            return (0, 0, 0, 100, recentResults, poolOpen, rebaseReady, rebaseRequested);
